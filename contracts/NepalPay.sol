...
contract NepalPay is Ownable {
    ...
    function contribute(address _campaignCreator, bytes32 _campaignId, uint256 _amount) external {
        require(crowdfundingCampaigns[_campaignCreator][_campaignId] > 0, "Campaign does not exist");
        require(_amount > 0, "Contribution amount must be greater than zero");
        token.transferFrom(msg.sender, _campaignCreator, _amount);
        emit ContributionMade(msg.sender, _campaignId, _amount);
    }

    // Function to set a scheduled payment
    function setScheduledPayment(address _recipient, uint256 _amount, uint256 _timestamp) external {
        require(_amount > 0, "Payment amount must be greater than zero");
        require(_timestamp > block.timestamp, "Payment timestamp must be in the future");
        require(balance[msg.sender] >= _amount, "Insufficient balance");

        bytes32 paymentId = keccak256(abi.encodePacked(_recipient, _amount, _timestamp));
        scheduledPayments[msg.sender][paymentId] = ScheduledPayment({
            amount: _amount,
    ...
    // Function to execute a scheduled payment
    function executeScheduledPayment(address _sender, address _recipient, uint256 _amount, uint256 _timestamp) external {
        bytes32 paymentId = keccak256(abi.encodePacked(_recipient, _amount, _timestamp));
        ScheduledPayment storage payment = scheduledPayments[_sender][paymentId];
        
        require(payment.active, "Scheduled payment is not active");
        require(payment.timestamp <= block.timestamp, "Scheduled payment timestamp is in the future");
        require(balance[_sender] >= payment.amount, "Insufficient balance");

        balance[_sender] -= payment.amount;
        balance[_recipient] += payment.amount;

        payment.active = false;
    }

    // Function to cancel a scheduled payment
    function cancelScheduledPayment(address _recipient, uint256 _amount, uint256 _timestamp) external {
        bytes32 paymentId = keccak256(abi.encodePacked(_recipient, _amount, _timestamp));
        ScheduledPayment storage payment = scheduledPayments[msg.sender][paymentId];
